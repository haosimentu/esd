...
var MU = {
	init: function(){
		...
		$("#userChoice li.item").on({//头部用户信息下拉菜单
			mouseenter: function(){
				...
				w = w<=120 ? 120 : w;
				$(this).find('div.user-info').css({'width':w,'left':0}).show();
				$("#userChoice li.item:last").find('div.user-info').css({'right':0,"left":"auto"})
			},
			mouseleave: function(){
	...
	msgTips: function(status,txt,timeout,callback){
		...
		var html = '<div class="msgbox_layer_wrap" id="msgTips" style="z-index:'+parseInt( new Date().getTime()/1000 )+'"><span class="msgbox_layer"><span class="icon_clear"></span><span class="icon '+status+'"></span>'+txt+'<span class="icon_end"></span></span></div>';
		$('body').append(html);
		if (flag) {
	...
	mask: function(){
		if($("#filter").length<=0){
			...
			var h=$("html").height() > document.documentElement.clientHeight ? $("html").height():document.documentElement.clientHeight;
			$("body").append(mybg);
			$("div#filter").height(h);
